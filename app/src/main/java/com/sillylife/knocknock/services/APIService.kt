...
object APIService {
    ...
    //    const val BASE_URL = "http://api.zoopzam.com/"
//    const val BASE_URL = "http://192.168.29.152:8080"
    const val BASE_URL = "https://api.ratrey.co/"

    //    const val BASE_URL = "http://13.126.22.193/"
    ...
    private fun interceptorWithOutBearer(): Interceptor {
        return Interceptor { chain ->
            ...
            if (response.code == 401) {
                handleUnauthorisedResponse(request.url.toUrl().path)
            }
            response
        }
    }
    ...
    private fun interceptor(cacheDuration: Long): Interceptor {
        return Interceptor { chain ->
            ...
            FirebaseAuthUserManager.retrieveIdToken(
                    false,
                    object : FirebaseAuthUserManager.TokenRetrieveListener {
